...
const DailyReportPanel = ({ open, onClose, item }) => {
  ...
  const ReportPreview = () => {
    const RP = {
      infoTh: { border:"1px solid #bfbfbf", padding:"3px 8px", fontSize:11, lineHeight:1.2, height:26, verticalAlign:"middle", background:"#fafafa", textAlign:"left", fontWeight:700, wordBreak:"break-word" },
      infoTd: { border:"1px solid #bfbfbf", padding:"3px 8px", fontSize:11, lineHeight:1.2, height:26, verticalAlign:"middle", wordBreak:"break-word" },
      infoLabel: { borderRight:"1px solid #bfbfbf", borderBottom:"1px solid #bfbfbf", padding:"3px 8px", fontSize:11, lineHeight:1.2, display:"flex", alignItems:"center", wordBreak:"break-word", minHeight:26, background:"#fafafa", fontWeight:700 },
      infoValue: { borderRight:"1px solid #bfbfbf", borderBottom:"1px solid #bfbfbf", padding:"3px 8px", fontSize:11, lineHeight:1.2, display:"flex", alignItems:"center", wordBreak:"break-word", minHeight:26, background:"#fff" },
      checkTh: { border:"1px solid #bfbfbf", padding:"5px 7px", fontSize:11, background:"#f5f5f5", textAlign:"center", fontWeight:700 },
      checkTd: { border:"1px solid #bfbfbf", padding:"4px 7px", fontSize:11, verticalAlign:"middle", wordBreak:"break-word", lineHeight:1.2 },
      signTh: { border:"1px solid #bfbfbf", background:"#f5f5f5", fontWeight:700, height:28, padding:"3px 8px", fontSize:11.5, textAlign:"center" },
      signTd: { border:"1px solid #bfbfbf", height:52, padding:"6px 8px", fontSize:11.5, verticalAlign:"middle" },
    };
    const chip = (bg, col) => ({ display:"inline-flex", alignItems:"center", justifyContent:"center", padding:"3px 9px", borderRadius:999, fontSize:11, fontWeight:700, lineHeight:1, whiteSpace:"nowrap", background:bg, color:col });
    const badge = (isAbn) => ({ display:"inline-block", minWidth:48, padding:"2px 8px", borderRadius:999, fontSize:10, fontWeight:700, lineHeight:1.2, textAlign:"center", whiteSpace:"nowrap", background:isAbn?"#fdeaea":"#e7f5eb", color:isAbn?"#d14343":"#197a4e" });
    const h2S = { margin:"0 0 7px", fontSize:15, fontWeight:700, letterSpacing:-0.2 };
    const reportDT = item?.submitDt && item?.submitDt !== "-" ? item.submitDt : "—";
    const ciList = inspItems.map((r,i) => ({ seq:i+1, name:r.nm, isAuto:r.method==="자동", val:r.val, isAbn:r.result==="비정상", note:r.errCode||"" }));
    const sysInfoItems = [
      { label:"정보시스템", value:item?.sysNm||"" },
      { label:"대상자원",   value:item?.resNm||"" },
      { label:"IP주소",     value:item?.ip||"" },
    ].filter(x=>x.value);
    return (
      <div style={{ fontFamily:'"Nanum Gothic","Malgun Gothic","Apple SD Gothic Neo",Arial,sans-serif', background:"#fff", padding:"20px 24px", color:"#111", fontSize:12, lineHeight:1.4 }}>
        <div style={{ textAlign:"center", marginBottom:14 }}>
          <h1 style={{ margin:0, fontSize:21, fontWeight:700, letterSpacing:-0.2 }}>점검일지</h1>
          <div style={{ marginTop:8, paddingTop:7, borderTop:"2px solid #111", fontSize:11, color:"#222" }}>{item?.resNm||""} · {item?.sysNm||""} · {reportDT}</div>
        </div>
        <div style={{ marginTop:16 }}>
          <h2 style={h2S}>점검 결과 요약</h2>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10, marginBottom:8 }}>
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              <span style={chip("#e7f5eb","#197a4e")}>정상 {normalCnt}건</span>
              <span style={chip("#fdeaea","#d14343")}>비정상 {abnCnt}건</span>
              <span style={chip("#edf3ff","#315ecc")}>전체 {inspItems.length}건</span>
            </div>
            <span style={{ fontSize:11, color:"#444", whiteSpace:"nowrap" }}>수행일시 : {item?.execDt||""}</span>
          </div>
        </div>
        <div style={{ marginTop:16 }}>
          <h2 style={h2S}>점검기본정보</h2>
          <table style={{ width:"100%", borderCollapse:"collapse", tableLayout:"fixed" }}>
            <colgroup><col style={{width:"19%"}}/><col style={{width:"31%"}}/><col style={{width:"17%"}}/><col style={{width:"33%"}}/></colgroup>
            <tbody>
              <tr><th style={RP.infoTh}>고객사명</th><td style={RP.infoTd}>—</td><th style={RP.infoTh}>점검유형</th><td style={RP.infoTd}>{item?.rptType||"일일"} / 평일</td></tr>
              <tr><th style={RP.infoTh}>점검자</th><td style={RP.infoTd}>{item?.insp||""}</td><th style={RP.infoTh}>점검표</th><td style={RP.infoTd}>{item?.clNm||""}</td></tr>
              <tr><th style={RP.infoTh}>점검일시</th><td style={RP.infoTd}>{item?.execDt||""}</td><th style={RP.infoTh}>보고일시</th><td style={RP.infoTd}>{reportDT}</td></tr>
            </tbody>
          </table>
        </div>
        <div style={{ marginTop:16 }}>
          <h2 style={h2S}>시스템 정보</h2>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", borderTop:"1px solid #bfbfbf", borderLeft:"1px solid #bfbfbf" }}>
            {sysInfoItems.length > 0 ? sysInfoItems.map((si,i) => (
              <div key={i} style={{ display:"grid", gridTemplateColumns:"120px 1fr" }}>
                <div style={RP.infoLabel}>{si.label}</div>
                <div style={RP.infoValue}>{si.value}</div>
              </div>
            )) : (
              <div style={{ gridColumn:"1 / -1", borderRight:"1px solid #bfbfbf", borderBottom:"1px solid #bfbfbf", padding:10, fontSize:11, textAlign:"center", color:"#888", background:"#fafafa", minHeight:40, display:"flex", alignItems:"center", justifyContent:"center" }}>정보 없음</div>
            )}
          </div>
        </div>
        <div style={{ marginTop:16 }}>
          <h2 style={h2S}>점검 체크사항</h2>
          <table style={{ width:"100%", borderCollapse:"collapse", tableLayout:"fixed" }}>
            <colgroup><col style={{width:"6%"}}/><col style={{width:"31%"}}/><col style={{width:"11%"}}/><col style={{width:"14%"}}/><col style={{width:"9%"}}/><col style={{width:"29%"}}/></colgroup>
            <thead><tr>{["연번","점검항목 명","점검유형","결과값","판정","비고"].map(h=><th key={h} style={RP.checkTh}>{h}</th>)}</tr></thead>
            <tbody>
              {ciList.length === 0 ? (
                <tr><td colSpan={6} style={{ ...RP.checkTd, textAlign:"center", color:"#888", padding:"14px 0", background:"#fafafa" }}>항목 없음</td></tr>
              ) : ciList.map(ci => (
                <tr key={ci.seq} style={{ background:ci.isAbn?"#fff8f8":"transparent" }}>
                  <td style={{ ...RP.checkTd, textAlign:"center" }}>{ci.seq}</td>
                  <td style={RP.checkTd}>{ci.name}</td>
                  <td style={{ ...RP.checkTd, textAlign:"center", color:ci.isAuto?"#2457c5":"#8a6112", fontWeight:700, whiteSpace:"nowrap" }}>{ci.isAuto?"자동":"육안"}</td>
                  <td style={{ ...RP.checkTd, textAlign:"center", color:ci.isAbn?"#d14343":"#197a4e", fontWeight:700 }}>{ci.val}</td>
                  <td style={{ ...RP.checkTd, textAlign:"center" }}><span style={badge(ci.isAbn)}>{ci.isAbn?"비정상":"정상"}</span></td>
                  <td style={RP.checkTd}>{ci.note}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div style={{ marginTop:16 }}>
          <h2 style={h2S}>주요 조치내역 및 특이사항</h2>
          <div style={{ minHeight:60, border:"1px dashed #c3c3c3", padding:10, fontSize:11.5, lineHeight:1.65, whiteSpace:"pre-wrap" }}>{item?.note||"특이사항 없음"}</div>
        </div>
        <div style={{ marginTop:16 }}>
          <h2 style={h2S}>육안 점검 사진</h2>
          <div style={{ display:"flex", gap:8, marginTop:8 }}>
            {eyeItems.length > 0 ? eyeItems.map((ei,i) => {
              const ph = eyeItemPhotos[ei.id];
              return (
                <div key={i} style={{ flex:1, height:92, border:"1px dashed #c3c3c3", display:"flex", alignItems:"center", justifyContent:"center", color:"#999", fontSize:11, background:"#fafafa", textAlign:"center", padding:6, overflow:"hidden" }}>
                  {ph?.url ? <img src={ph.url} alt={ei.nm} style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }}/> : <span>{ei.nm}</span>}
                </div>
              );
            }) : (
              <div style={{ flex:"1 1 100%", height:92, border:"1px dashed #c3c3c3", display:"flex", alignItems:"center", justifyContent:"center", color:"#999", fontSize:11, background:"#fafafa" }}>사진 없음</div>
            )}
          </div>
        </div>
        <div style={{ margin:"18px 0 8px", fontSize:14 }}>위와 같이 점검을 하였음을 확인 합니다.</div>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          <colgroup><col style={{width:"25%"}}/><col style={{width:"25%"}}/><col style={{width:"25%"}}/><col style={{width:"25%"}}/></colgroup>
          <tbody>
            <tr><th colSpan={2} style={RP.signTh}>점검자</th><th colSpan={2} style={RP.signTh}>확인자</th></tr>
            <tr>
              <td style={{ ...RP.signTd, borderRightColor:"transparent" }}></td>
              <td style={{ ...RP.signTd, borderLeftColor:"transparent", textAlign:"right" }}><span style={{ fontSize:15, fontWeight:700, whiteSpace:"nowrap" }}>{item?.insp||""}</span> (인)</td>
              <td style={{ ...RP.signTd, borderRightColor:"transparent" }}></td>
              <td style={{ ...RP.signTd, borderLeftColor:"transparent", textAlign:"right" }}><span style={{ fontSize:15, fontWeight:700, whiteSpace:"nowrap" }}></span> (인)</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const ChecklistPanel = ({ open, onClose, item, linkedResIds = [], onLinkChange, onSaved, initialTab = "info", isJustCreated = false, resLinkMap = {} }) => {
  ...
  const PreviewDoc = () => {
    ...
    const tdStr = `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,"0")}-${String(today.getDate()).padStart(2,"0")}`;
    const RP = {
      infoTh: { border:"1px solid #bfbfbf", padding:"3px 8px", fontSize:11, lineHeight:1.2, height:26, verticalAlign:"middle", background:"#fafafa", textAlign:"left", fontWeight:700, wordBreak:"break-word" },
      infoTd: { border:"1px solid #bfbfbf", padding:"3px 8px", fontSize:11, lineHeight:1.2, height:26, verticalAlign:"middle", wordBreak:"break-word" },
      infoLabel: { borderRight:"1px solid #bfbfbf", borderBottom:"1px solid #bfbfbf", padding:"3px 8px", fontSize:11, lineHeight:1.2, display:"flex", alignItems:"center", wordBreak:"break-word", minHeight:26, background:"#fafafa", fontWeight:700 },
    ...
    const sysInfoItems = form.exposedRes.length > 0
      ? form.exposedRes.map(r => ({ label: r, value: "" }))
      : [];
    return (
      <div style={{ fontFamily:'"Nanum Gothic","Malgun Gothic","Apple SD Gothic Neo",Arial,sans-serif', background:"#fff", padding:"20px 24px", color:"#111", fontSize:12, lineHeight:1.4 }}>
        <div style={{ textAlign:"center", marginBottom:14 }}>
          <h1 style={{ margin:0, fontSize:21, fontWeight:700, letterSpacing:-0.2 }}>점검일지</h1>
          <div style={{ marginTop:8, paddingTop:7, borderTop:"2px solid #111", fontSize:11, color:"#222" }}>
            {form.inspType||""} · {form.inspKind||""} · {tdStr}
          </div>
        </div>
        <div style={{ marginTop:16 }}>
          <h2 style={h2S}>점검 결과 요약</h2>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10, marginBottom:8 }}>
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              <span style={chip("#e7f5eb","#197a4e")}>정상 0건</span>
              <span style={chip("#fdeaea","#d14343")}>비정상 0건</span>
              <span style={chip("#edf3ff","#315ecc")}>전체 {totalCnt}건</span>
            </div>
            <span style={{ fontSize:11, color:"#444", whiteSpace:"nowrap" }}>작업일자 : {tdStr}</span>
          </div>
        </div>
        <div style={{ marginTop:16 }}>
          <h2 style={h2S}>점검기본정보</h2>
          <table style={{ width:"100%", borderCollapse:"collapse", tableLayout:"fixed" }}>
            <colgroup><col style={{width:"19%"}}/><col style={{width:"31%"}}/><col style={{width:"17%"}}/><col style={{width:"33%"}}/></colgroup>
            <tbody>
              <tr><th style={RP.infoTh}>고객사명</th><td style={RP.infoTd}></td><th style={RP.infoTh}>점검유형</th><td style={RP.infoTd}>{form.inspType||""} / 평일</td></tr>
              <tr><th style={RP.infoTh}>점검자</th><td style={RP.infoTd}></td><th style={RP.infoTh}>점검표</th><td style={RP.infoTd}>{form.nm||""}</td></tr>
              <tr><th style={RP.infoTh}>점검일시</th><td style={RP.infoTd}>{tdStr}</td><th style={RP.infoTh}>보고일시</th><td style={RP.infoTd}></td></tr>
            </tbody>
          </table>
        </div>
        <div style={{ marginTop:16 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", borderTop:"1px solid #bfbfbf", borderLeft:"1px solid #bfbfbf" }}>
            {sysInfoItems.length > 0 ? sysInfoItems.map((si,i) => (
              <div key={i} style={{ display:"grid", gridTemplateColumns:"120px 1fr" }}>
                <div style={RP.infoLabel}>{si.label}</div>
                <div style={RP.infoValue}></div>
              </div>
            )) : (
              <div style={{ gridColumn:"1 / -1", borderRight:"1px solid #bfbfbf", borderBottom:"1px solid #bfbfbf", padding:10, fontSize:11, textAlign:"center", color:"#888", background:"#fafafa", minHeight:40, display:"flex", alignItems:"center", justifyContent:"center" }}>노출 자원정보를 선택하면 여기에 표시됩니다</div>
            )}
          </div>
        </div>
        <div style={{ marginTop:16 }}>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", tableLayout:"fixed" }}>
            ...
            <tbody>
              {allItems.length === 0 ? (
                <tr><td colSpan={6} style={{ ...RP.checkTd, textAlign:"center", color:"#888", padding:"14px 0", background:"#fafafa" }}>점검항목을 추가하면 여기에 표시됩니다</td></tr>
              ) : allItems.map((it,i) => (
                <tr key={it.id}>
                  <td style={{ ...RP.checkTd, textAlign:"center" }}>{i+1}</td>
                  <td style={RP.checkTd}>{it.nm}</td>
                  <td style={{ ...RP.checkTd, textAlign:"center", color:it.method==="자동"?"#2457c5":"#8a6112", fontWeight:700, whiteSpace:"nowrap" }}>{it.method==="자동"?"자동":"육안"}</td>
                  <td style={{ ...RP.checkTd, textAlign:"center", color:"#888" }}>{it.method==="자동"?"(자동 수집)":"□ 정상 / □ 비정상"}</td>
                  <td style={{ ...RP.checkTd, textAlign:"center", color:"#888" }}>—</td>
                  <td style={RP.checkTd}></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div style={{ marginTop:16 }}>
          <h2 style={h2S}>주요 조치내역 및 특이사항</h2>
          <div style={{ minHeight:60, border:"1px dashed #c3c3c3", padding:10, fontSize:11.5, lineHeight:1.65, whiteSpace:"pre-wrap", color: form.memo ? "#111" : "#aaa", fontStyle: form.memo ? "normal" : "italic" }}>
        ...
        <div style={{ marginTop:16 }}>
          ...
          <div style={{ display:"flex", gap:8, marginTop:8 }}>
            {[1,2,3].map(n => (
              <div key={n} style={{ flex:1, height:92, border:"1px dashed #c3c3c3", display:"flex", alignItems:"center", justifyContent:"center", color:"#999", fontSize:11, background:"#fafafa", textAlign:"center" }}>사진 {n}</div>
            ))}
          </div>
        </div>
        <div style={{ margin:"18px 0 8px", fontSize:14 }}>위와 같이 점검을 하였음을 확인 합니다.</div>
  ...
  return (
    <SidePanel open={open} onClose={onClose}
      ...
      {showPreview && (() => {
        ...
        const eyeList = inspItems.filter(r=>r.method==="육안");
        return (
          <div style={{ flex:1, minWidth:0, borderRight:`1px solid ${C.brd}`, display:"flex", flexDirection:"column", overflow:"hidden" }}>
            <div style={{ padding:"14px 20px 12px", borderBottom:`1px solid ${C.brd}`,
              display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0, background:"#FAFBFC" }}>
              ...
              <div style={{ border:`1px solid ${C.brd}`, borderRadius:6, overflow:"hidden", boxShadow:"0 2px 12px rgba(0,0,0,.06)" }}>
                <div style={{ fontFamily:'"Nanum Gothic","Malgun Gothic","Apple SD Gothic Neo",Arial,sans-serif', background:"#fff", padding:"20px 24px", color:"#111", fontSize:12, lineHeight:1.4 }}>
                  ...
                  <div style={{ marginTop:16 }}>
                    <h2 style={h2S}>육안 점검 사진</h2>
                    <div style={{ display:"flex", gap:8, marginTop:8 }}>
                      {eyeList.length > 0 ? eyeList.map((ei,i) => (
                        <div key={i} style={{ flex:1, height:92, border:"1px dashed #c3c3c3", display:"flex", alignItems:"center", justifyContent:"center", color:"#999", fontSize:11, background:"#fafafa", textAlign:"center", padding:6 }}>
                          <span>{ei.nm}</span>
                        </div>
                      )) : (
                        <div style={{ flex:"1 1 100%", height:92, border:"1px dashed #c3c3c3", display:"flex", alignItems:"center", justifyContent:"center", color:"#999", fontSize:11, background:"#fafafa" }}>사진 없음</div>
